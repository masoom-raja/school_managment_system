...
export default function Profile() {
  ...
  console.log(currentUser.rest._id,"current user");
  useEffect(()=>{
    if(file){
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let updatefromdata={rest:data}
      console.log(data,"responsing data")
      console.log(updatefromdata,"update data")
      if (data.success === false) {
        dispatch(updateUserFailure(data.message));
        return;
      }

      dispatch(updateUserSuccess(updatefromdata));
  ...
  return (
    <div className='p-3 max-w-lg mx-auto' >
      ...
      <form onSubmit={handleSubmit} className='flex flex-col gap-4'>
        ...
        <img onClick={()=>fileRef.current.click()} src={formData.avatar || currentUser.rest.avatar} 
        alt='profile' 
